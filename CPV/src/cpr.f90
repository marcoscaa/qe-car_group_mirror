...
SUBROUTINE cprmain( tau_out, fion_out, etot_out )
  ...
  USE funct,                    ONLY : dft_is_meta !HK/MCA
  ...
  !
  !for cp dynamics conv_elec is set to true
  !
  conv_elec=.true.
  !
  !======================================================================
  ...
  main_loop: DO
     ...
     !=======================================================================
     !
     !    CP-BO electron minimization ... 
     !
     !=======================================================================
     !
     IF(tcpbo) CALL cp_bo_emin(tfirst,tlast,enb,enbi,ccc,stress,bigr,iter)
     !
     !=======================================================================
     !
     ...
     IF ( tpre ) THEN
        ...
        !
        IF (tstdout) THEN
          WRITE(stdout,'(5X,"Total Electronic Pressure (GPa)",F20.5,I7)') &
...
444  IF ( tfor ) THEN
        ...
        !
        CALL plugin_ext_forces(stress) !HK/BS: plumed2 support
        !
        !
     ...
     IF ( tconv ) THEN
        ...
        IF ( ionode ) THEN
           ...
           WRITE( stdout,1949)
           WRITE( stdout,1951) ekinc 
           WRITE( stdout,1952) ekin 
           WRITE( stdout,1953) epot 
           WRITE( stdout,1954) etot 
           WRITE( stdout,1955) tempp 
           WRITE( stdout,1956) enthal 
           WRITE( stdout,1957) econs 
           WRITE( stdout,1990)
 1949      FORMAT(//,3X,'Physical Quantities After Minimization')
 1951      FORMAT(3X,'!ekinc         : ',F16.8,' (AU)')
 1952      FORMAT(3X,'!ekin          : ',F16.8,' (AU)')
 1953      FORMAT(3X,'!epot          : ',F16.8,' (AU)')
 1954      FORMAT(3X,'!total energy  : ',F16.8,' (AU)')
 1955      FORMAT(3X,'!temperature   : ',F16.8,' (K )')
 1956      FORMAT(3X,'!enthalpy      : ',F16.8,' (AU)')
 1957      FORMAT(3X,'!econs         : ',F16.8,' (AU)')
 1990      FORMAT(/)
           !
        END IF
        !
     END IF
     !
     ! to avoid printing Wannier related quantities here during CP-BO dynamics 
     !
     IF(tcpbo) conv_elec=.FALSE.
     !
     IF ( lwf ) &
     ...
     !
     IF(tcpbo) conv_elec=.TRUE.
     !
     CALL stop_clock( 'cpr_md' )   ! swaped this line with EXIT | corrected
     !
...
SUBROUTINE terminate_run()
  ...
  USE control_flags,     ONLY : ts_vdw, thdyn, tortho, tcpbo, ldriver
  ...
  IF ( dft_is_hybrid().AND.exx_is_active() ) THEN
    ...
    CALL print_clock('getselfv')         ! calculation pair potential
    ...
    CALL print_clock('send_v')           ! communication
    CALL print_clock('send_v_wait')      ! communication
...
!==============================================================
  ...
  !=============================================================
  IF (tcpbo) THEN
    WRITE( stdout, '(/5x,"Called by CP_BO:")' )
    CALL print_clock( 'cp_bo' )
  END IF
  !=============================================================
  IF (ldriver) THEN
    WRITE( stdout, '(/5x,"Called by DRIVER:")' )
    CALL print_clock( 'driver_md' )
  END IF
  !=============================================================
  !
