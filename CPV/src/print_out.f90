...
!=----------------------------------------------------------------------------=!
      ...
      !
      USE kinds,             ONLY : DP
      USE control_flags,     ONLY : iprint, textfor, do_makov_payne, conv_elec,tcpbo
      ...
      IF( .not. tcg ) THEN
         ...
         IF(dft_is_hybrid().AND.exx_is_active()) THEN
           ...
           IF (ts_vdw) THEN
             ...
             IF(tcpbo) THEN
               ! 
               WRITE(stdout,19483) nfi,temphc,tempp,etot,enthal,econs,econt,-exx*exxalfa,EtsvdW
               ! 
             ELSE
               ! 
...
1947  FORMAT( 4X,'nfi',5X,'ekinc',14X,'temph',2X,'tempp',5X,'etot',17X,'enthal', &
           & 15X,'econs',16X,'econt',14X,'vnhh',5X,'xnhh0',4X,'vnhp',5X,'xnhp0')            ! GGA 
19470  FORMAT(4X,'nfi',5X,'ekinc',14X,'temph',2X,'tempp',10X,'exx',15X,'etot',15X,'enthal', &
           & 15X,'econs',16X,'econt',12X,'vnhh',5X,'xnhh0',4X,'vnhp',5X,'xnhp0')            ! Hybrid 
19471  FORMAT(4X,'nfi',5X,'ekinc',14X,'temph',2X,'tempp',5X,'etot',17X,'enthal', &
           & 15X,'econs',16X,'econt',16X,'vnhh',5X,'xnhh0',4X,'vnhp',5X,'xnhp0',8X,'evdw')  ! GGA+vdW
19472  FORMAT(4X,'nfi',5X,'ekinc',14X,'temph',2X,'tempp',10X,'exx',15X,'etot',15X,'enthal', &
           & 15X,'econs',16X,'econt',16X,'vnhh',5X,'xnhh0',4X,'vnhp',5X,'xnhp0',8X,'evdw')  ! Hybrid+vdW
19473  FORMAT(4X,'nfi',8X,'temph',8X,'tempp',15X,'etot',15X,'enthal',15X,'econs', &
...
!19482  FORMAT( I7,1X,F20.15,1X,F6.1,1X,F7.2,5(1X,F20.12),4(1X,F8.4),1X,F20.12 ) ! Hybrid+vdW                        
1948   FORMAT( I7,1X,ES12.6,1X,F6.1,1X,F7.2,4(1X,F20.12),4(1X,F8.4) )  ! GGA 
19480  FORMAT( I7,1X,ES12.6,1X,F6.1,1X,F7.2,5(1X,F20.12),4(1X,F8.4) )  ! PBE0                       
19481  FORMAT( I7,1X,ES12.6,1X,F6.1,1X,F7.2,4(1X,F20.12),4(1X,F8.4),1X,F20.12 ) ! GGA+vdW
19482  FORMAT( I7,1X,ES12.6,1X,F6.1,1X,F7.2,5(1X,F20.12),4(1X,F8.4),1X,F20.12 ) ! Hybrid+vdW                        
19483  FORMAT( I7,2(2X,ES12.6),6(1X,F18.10) )

29471 FORMAT( '#',3X,'nfi',4X,'time(ps)',8X,'ekinc',8X,'T_cell(K)',5X,'T_ion(K)',10X,'etot',15X,'enthal',15X,'econs', &
...
!=----------------------------------------------------------------------------=!
    ...
    IF(tcpbo) THEN
      WRITE( stdout, *) '  NFE    [int]          - CP-BO electronic step index'
      WRITE( stdout, *) '  MAD_DFION [HARTREE A.U.] - Mean absolute deviations in ionic forces during &
                         & CP-BO electronic minimization'
    END IF
    WRITE( stdout, *) 
    !
    RETURN
