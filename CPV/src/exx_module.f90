...
CONTAINS
  ...
  !--------------------------------------------------------------------------------------------------------------
  SUBROUTINE exx_pot_derivative (np_in_sp_me, n, v, Jim, dvdr_out)
      !
      ...
      !                                             |d/d y| = [J]  |d/d b|
      !                                             [d/d z]        [d/d c]
      REAL(DP), INTENT(OUT) :: dvdr_out(n,3)
      ! loc
      INTEGER               :: i, ish, ii, jj, kk
      REAL(DP)              :: v1, v2, v3
      REAL(DP)              :: dvdr(n,3) ! tmp storage (a,b,c) space derivatives
      !
      !  --------------------------------------------------------
      !  initialize
      dvdr(:,:) = 0.0_DP
      ...
      DO i = 1, n ! running inside PS sphere
        dvdr_out(i,1) = Jim(1,1)*dvdr(i,1) + Jim(1,2)*dvdr(i,2) + Jim(1,3)*dvdr(i,3)
        dvdr_out(i,2) = Jim(2,1)*dvdr(i,1) + Jim(2,2)*dvdr(i,2) + Jim(2,3)*dvdr(i,3)
        dvdr_out(i,3) = Jim(3,1)*dvdr(i,1) + Jim(3,2)*dvdr(i,2) + Jim(3,3)*dvdr(i,3)
      END DO
      !$omp end parallel do
      !
      RETURN
  ...
  SUBROUTINE exx_energy_cell_derivative (np_in_sp_me, n, tran, &
          vl, ha_proj, hb_proj, hc_proj, rhol, Jim, dexx_dhab)
      !
      IMPLICIT NONE
      ! io
      ...
      REAL(DP), INTENT(IN)  :: ha_proj(3), hb_proj(3), hc_proj(3)
      REAL(DP), INTENT(IN)  :: Jim(3,3)  ! jacobian [d/d x]        [d/d a]
      !                                             |d/d y| = [J]  |d/d b|
      !                                             [d/d z]        [d/d c]
      ...
      !
      CALL exx_pot_derivative (np_in_sp_me, n, vl, Jim, dvdr)
      !
      ! Integration:
      ...
      DO i = 1, n ! running inside PS sphere
        ...
        !
        r_alpha(1) = DBLE(ii)*ha_proj(1)+DBLE(jj)*hb_proj(1)+DBLE(kk)*hc_proj(1)
        r_alpha(2) = DBLE(ii)*ha_proj(2)+DBLE(jj)*hb_proj(2)+DBLE(kk)*hc_proj(2)
        r_alpha(3) = DBLE(ii)*ha_proj(3)+DBLE(jj)*hb_proj(3)+DBLE(kk)*hc_proj(3)
        !
