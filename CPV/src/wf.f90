...
SUBROUTINE wf( clwf, c, bec, eigr, eigrb, taub, irb, &
  ...
  USE control_flags,            ONLY : iverbosity,conv_elec,tcpbo
...
!               & 'Number of Processors is greater than the number of states', 1 )
  ...
  !write(1200+me,*) g_start, g_end !HK: debug
  !
#else
  g_start = 1
  g_end   = ntot
...
!$omp end parallel do 
  ...
  ! HK dbug
  !if(me.eq.1) write (1000,*) indexplus
  !if(me.eq.1) write (1001,*) indexminus
  !if(me.eq.1) write (1002,*) tag      
  !if(me.eq.1) write (1003,*) tagp     
  !CALL mp_barrier( intra_bgrp_comm )
#endif
  DEALLOCATE(bigg)
  DEALLOCATE(bign)
...
! ouput: the unitary transformation matrix U
       ...
       USE cp_main_variables, ONLY : nfi, iprint_stdout
       USE time_step,         ONLY : tps
       USE control_flags,     ONLY : conv_elec,tcpbo 
       USE parallel_include
       USE io_global,         ONLY : ionode, stdout
