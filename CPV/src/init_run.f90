...
SUBROUTINE init_run()
  ...
  IF ( dft_is_hybrid() .AND. lwf ) THEN
    ...
    CALL exx_initialize()
    !
  else if ( dft_is_hybrid()) THEN
    !
    !write(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
    !write(*,*) '!! WARNING WARNING WARNING  !!'
    !write(*,*) '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
    !write(*,*) 'You might want to use wannier-function based hybrid algorithm...'
    call errore('init_run','exact exchage in CP currently depend on Wannier functions...',1)
    !
  END IF
  !
