...
MODULE input
   ...
   SUBROUTINE iosys_pseudo()
     ...
     USE input_parameters,        ONLY : atom_pfile, pseudo_dir, ntyp, nat, &
                                         prefix, outdir, input_dft, srvaddress
...
!exx_wf related
     lwf = ( TRIM( calculation ) == 'cp-wf'      .OR. &
             ...
             TRIM( calculation ) == 'driver-vc-cp-wf')
     lwfnscf     = ( TRIM( calculation ) == 'cp-wf-nscf' )
     lwfpbe0nscf = ( dft_is_hybrid() .AND. lwfnscf  )
!====================================================================
     ! 
     ! driver options ... need more?? 
     !
     ldriver = ( TRIM( calculation ) == 'driver'      .OR. &
...
!         cell temperature control of ions via Nose' thermostat
      ...
      !overwrite controls in calculation = driver mode
      IF(ldriver) THEN
        !
        tfor_ = .TRUE.
        tpre_ = .TRUE.
        !
        IF(TRIM( calculation ) == 'driver-vc-cp-wf') thdyn_ = .TRUE.
        IF(TRIM( calculation ) == 'driver-vc') thdyn_ = .TRUE.
        !
      END IF
      !
...
511 FORMAT(   3X,'emass cut-off                      = ',F10.2)
  ...
  SUBROUTINE modules_info()
    ...
    USE control_flags, ONLY:  tortho, tnosee, trane, ampre, &
                              ...
                              thdyn, tnoseh, tcpbo
    !
    USE cp_bo_emin_module,    ONLY: cp_bo_print_info
    !
    ...
    IF( ionode ) THEN
      ...
      ELSE IF( TRIM(electron_dynamics) == 'cg' ) THEN
          WRITE( stdout,511)
      ELSE IF( TRIM(electron_dynamics) == 'cp-bo' ) THEN
          WRITE( stdout,516)
      ELSE
      ...
      !
      IF (tcpbo) CALL cp_bo_print_info( )
      !
...
515 FORMAT( 3X,'initial random displacement of el. coordinates with ',   &
               ' amplitude=',f10.6)
516 FORMAT( 3X,'Electron dynamics with CP-BO')
