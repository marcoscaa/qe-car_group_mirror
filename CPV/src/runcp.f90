...
!=----------------------------------------------------------------------------------=!
   ...
   SUBROUTINE runcp_uspp_x &
      ...
      use control_flags,       only : lwf, tsde, tcpbo
      ...
      USE input_parameters,    ONLY : dt_emin,emass_emin
      !
      IMPLICIT NONE
      !
     ...
     IF(tcpbo.EQV..TRUE.) THEN
       IF(iflag.EQ.1) dt2bye=(dt_emin*dt_emin/emass_emin)
     END IF
