SUBROUTINE getvofr( np_in_sp_me, np_in_sp, hcub, rho, v, tSelf, &
                    guess_state, psgsn, rho_old, v_old, cgstep)
    !=======================================================================================
    ...
    INTEGER                :: guess_state
    INTEGER                :: psgsn
    REAL(DP)               :: v_old  (np_in_sp, psgsn)
    REAL(DP)               :: rho_old(np_in_sp, psgsn)
    INTEGER                :: cgstep
    !-----------------------------------------------------------------------
    ...
    REAL(DP), ALLOCATABLE  :: GS_v  (:,:)
    REAL(DP), ALLOCATABLE  :: GS_coe(:)
    REAL(DP), ALLOCATABLE  :: GS_tmp(:)
    REAL(DP)               :: Cex(3)
    REAL(DP)               :: tmp1, tmp2
    ...
    ! --- initialize ---
    !-----------------------------------------------------------------------
    ALLOCATE( v_in_sp(np_in_sp_me)     )
    ALLOCATE( GS_rho (np_in_sp, psgsn) )
    ALLOCATE( GS_v   (np_in_sp, psgsn) )
    ALLOCATE( GS_coe (psgsn)           )
    ALLOCATE( GS_tmp (psgsn)           )
    !
    v_in_sp = 0.0D0
    Cex     = 0.0D0
    GS_rho  = 0.0D0
    ...
    DO itr = 1, gsn
      !--------------------------------------
      GS_rho(:,itr) = rho_old(:,itr)
      GS_v  (:,itr) = v_old  (:,itr)
      !--------------------------------------
      DO jtr = 1, itr-1
        GS_tmp(jtr) = DOT_PRODUCT(GS_rho(:,itr), GS_rho(:,jtr))
      END DO
      !--------------------------------------
      DO jtr = 1, itr-1
        GS_rho(:,itr) = GS_rho(:,itr) - GS_tmp(jtr) * GS_rho(:,jtr)
        GS_v  (:,itr) = GS_v  (:,itr) - GS_tmp(jtr) * GS_v  (:,jtr)
      END DO
      !--------------------------------------
      tmp1 = dnrm2(np_in_sp, GS_rho(:,itr), 1)
      GS_rho(:,itr) = GS_rho(:,itr) / tmp1
      GS_v  (:,itr) = GS_v  (:,itr) / tmp1
      !--------------------------------------
    END DO
    !---------------------------------------------------------------------------
    ...
    !---------------------------------------------------------------------------
    DO itr = 1, gsn
      GS_coe(itr) = DOT_PRODUCT(rho, GS_rho(:,itr))
    END DO
    !---------------------------------------------------------------------------
    DO itr = 1, gsn
      DO jtr = 1, np_in_sp
        v_in_sp(jtr) = v_in_sp(jtr) + GS_coe(itr) * GS_v(jtr,itr)
      END DO
    END DO
    !---------------------------------------------------------------------------
    ...
    DO itr = ngsn, 2, -1
      rho_old(:,itr) = rho_old(:,itr-1)
      v_old  (:,itr) = v_old  (:,itr-1)
    END DO
    !---------------------------------------------------------------------------
    DO itr = 1, np_in_sp
      rho_old(itr,1) = rho(itr)
      v_old  (itr,1) = v_in_sp(itr)
    END DO
    !---------------------------------------------------------------------------
