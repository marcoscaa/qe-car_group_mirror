...
!
      ...
      COMPLEX(DP), ALLOCATABLE :: psi_storage(:) !HK/MCA
...
!=======================================================================
      IF( dffts%have_task_groups ) THEN
         ...
         if (dft_is_meta()) ALLOCATE( psi_storage( dffts%tg_nnr * dffts%nogrp ) ) !HK/MCA
      ELSE
         nogrp_ = 1
         ALLOCATE( psi( dffts%nnr ) )
         if (dft_is_meta()) ALLOCATE( psi_storage( dffts%nnr ) ) !HK/MCA
      END IF
      !
      ci = ( 0.0d0, 1.0d0 )
      !
...
!$omp do 
               ...
               END DO
!$omp end do
               !HK/MCA : taskgroup parallelization of metaGGA should be done here (TODO)
               igno = igno + ngw
            ELSE
!$omp do 
               DO ig=1,ngw
               ...
               END DO
!$omp end do
               !HK/MCA : non-taskgroup parallelization of metaGGA should be moved here (TODO)
            END IF
         END IF
...
!$omp end parallel 
      ...
      IF(dft_is_meta()) THEN
         ...
         psi_storage = psi !HK/MCA : the psi contains -(V - V_xc^{kedtau}) psi_i and will be zeroed out in dforce_meta (store it here)
         CALL dforce_meta(c(1,i),c(1,i+1),df,da,psi,iss1,iss2,fi,fip) !METAGGA
         psi = psi + psi_storage !HK/MCA : put back -(V - V_xc^{kedtau}) psi_i with -V_xc^{kedtau} psi_i
      END IF
...
!
      ...
      DEALLOCATE( psi )
      if (dft_is_meta()) DEALLOCATE( psi_storage ) !HK/MCA
!
