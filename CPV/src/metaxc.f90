...
!
!
SUBROUTINE exch_corr_meta(nnr, nspin,grho,rho,kedtau,etxc)
  !--------------------------------------------------------------------
  use kinds, only: dp
  use funct, only: tau_xc, tau_xc_array, tau_xc_array_spin
  use fft_base,               only : dfftp         !HK/MCA
  use fft_interfaces,         only : fwfft, invfft !HK/MCA
  use gvect,                  only : ngm, nlm, nl  !HK/MCA
  IMPLICIT NONE
  !
  ! input
  !
  ...
  !
  ! output: excrho: exc * rho ;  etxc = \int excrho(r) d_r
  ...
  ! internal variables
  !
  complex(dp) :: psi(nnr)      !HK/MCA : buffer for fourier filtering
  INTEGER     :: k, ipol, is
  ...
  !
  !IF (nspin .ne. 1) call errore("metaxc", "nspin .ne.1 not implemented",1)
  !
...
!  !--------------------------------------------------------------------
!  use kinds, only: dp
!  use funct, only: tau_xc, tau_xc_array, tau_xc_spin
!  use fft_base,               only : dfftp         !HK/MCA
!  use fft_interfaces,         only : fwfft, invfft !HK/MCA
!  use gvect,                  only : ngm, nlm, nl  !HK/MCA
!  IMPLICIT NONE
...
!  ! output: rhor:   contains the exchange-correlation potential
!  !
!  !real(dp)  sxc_sto(nnr)   !HK/MCA : storage of sx+sc for fourier filtering
!  complex(dp) psi(nnr)           !HK/MCA : buffer for fourier filtering
!  real(dp)  etxc
...
!  !
!  !sxc_sto = 0._dp !HK/MCA : init
!  etxc = 0.d0
!  ! calculate the gradient of rho+rho_core in real space
!  !$omp parallel do &
!  !$omp    private(k,is,ipol,grho2,arho,segno,atau,sx,v1x,v2x,v3x,sc,v1c,v2c,v3c, &
!  !$omp            rh,grhoup,grhodw,v1xup,v1xdw,v2xup,v2xdw,v3xup,v3xdw, &
!  !$omp            v1cup,v1cdw,v2cup,v2cdw,v3cup,v3cdw ), reduction(+:etxc)
!  DO k = 1, nnr
!     DO is = 1, nspin
...
!#endif
!        ! HK: does not need the small number filter in case of libxc related
!        !     metaGGA routines
...
!           ENDDO
!           etxc = etxc +  (sx + sc) * segno  !HK/MCA: filter instead
!           !sxc_sto(k) = (sx + sc) * segno  !HK/MCA : storing for filtering later
!#ifndef __LIBXC
!        ELSE  
!           DO ipol = 1, 3  
!              grho (k, ipol, 1) = 0.d0  
...
!        !
!        DO ipol = 1, 3  
!           grho(k,ipol,1) = (v2xup(:) + v2cup(:))*grho(k,ipol,1) !MCA: v2 is a vector 
!           grho(k,ipol,2) = (v2xdw(:) + v2cdw(:))*grho(k,ipol,2)
!        ENDDO
!        kedtau(k,1)=  (v3xup + v3cup) *0.5d0
!        kedtau(k,2)=  (v3xdw + v3cdw) *0.5d0
!        etxc = etxc +  (sx + sc) !HK/MCA: filter instead
!        !sxc_sto(k) = (sx + sc)  !HK/MCA : storing for filtering later
!     ENDIF
!  ENDDO
!  !$omp end parallel do
!  !
