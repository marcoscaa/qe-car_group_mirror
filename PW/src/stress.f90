...
subroutine stress ( sigma ) 
  ...
  use mbdvdw_module, ONLY : HmbdvdW
  ...
  real(DP) :: sigmakin (3, 3), sigmaloc (3, 3), sigmahar (3, 3), &
       ...
       sigma_ts(3,3), sigma_mbd(3,3), H_tmp(3,3), lat_tmp(3,3)
  integer :: l, m
  !
  ...
  !
  !  MCA: metagga contribution 
  !
  call stres_mgga ( rho%of_r, rho%of_g, rho_core, rhog_core, rho%kin_r, &
                    dfftp%nr1, dfftp%nr2, dfftp%nr3, dfftp%nnr, nl, &
                    ngm, g, omega, sigmaxc)
  !
...
!  call stress_ion_efield (sigmaion )
   ...
   if(ts_vdw) then
     H_tmp = 0.0_DP
     H_tmp = HtsvdW*2.0_DP
     !lat_tmp = h      !HK : h is not update in PW
     lat_tmp = alat*at !HK : use at*alat instead
     sigma_ts = -1.0_DP*MATMUL( H_tmp, TRANSPOSE( lat_tmp ) )/ omega
   ...
   if(mbd_vdw) then
     ! Converts to Ry
     H_tmp = 0.0_DP
     H_tmp = HmbdvdW*2.0_DP
     !lat_tmp = h      !HK : h is not update in PW
     lat_tmp = alat*at !HK : use at*alat instead
     sigma_mbd = 1.0_DP*MATMUL( H_tmp, lat_tmp )/omega
   end if
