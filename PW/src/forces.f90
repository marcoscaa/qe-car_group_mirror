...
SUBROUTINE forces()
  ...
  USE control_flags, ONLY : gamma_only, remove_rigid_rot, textfor, &
                            iverbosity, llondon, lxdm, ts_vdw, mbd_vdw
  USE plugin_flags
  ...
  USE mbdvdw_module, ONLY : FmbdvdW
  !
  IMPLICIT NONE
  !
...
!
  ...
  DO ipol = 1, 3
     ...
     DO na = 1, nat
        ...
        if ( mbd_vdw ) force(ipol,na) = force(ipol,na) + 2.0_dp*FmbdvdW(na,ipol)
  ...
  !!
  !CALL plugin_ext_forces() !HK/BS: plumed2 should be used in the run_pwscf.f90 (check..)
  !!
