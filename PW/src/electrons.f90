...
SUBROUTINE electrons_scf ( no_printout )
  ...
  USE control_flags,        ONLY : mixing_beta, tr2, ethr, niter, nmix, &
                                   ...
                                   llondon, scf_must_converge, lxdm, ts_vdw, mbd_vdw
  USE io_files,             ONLY : iunwfc, iunmix, nwordwfc, output_drho, &
                                   iunres, iunefield, seqopn
  USE buffers,              ONLY : save_buffer, close_buffer
  ...
  USE mbdvdw_module,        ONLY : EmbdvdW, mbd_conv_elec, mbd_first_step
  !
  USE paw_variables,        ONLY : okpaw, ddd_paw, total_core_energy, only_paw
  USE paw_onecenter,        ONLY : PAW_potential
  USE paw_symmetry,         ONLY : PAW_symmetrize_ddd
  ...
  IF (ts_vdw.or.mbd_vdw) THEN
    ! HK: turn on screening flag to precompute ion position dependent quantities just once during SCF
    vdw_lscreen=.TRUE.
    vdw_lstep0=.FALSE.
  END IF
  !
  ...
  DO idum = 1, niter
     ...
     ! add the TS-vdW dispersion energy contribution to the total
     IF (ts_vdw) THEN
        ! factor 2 converts from Ha to Ry units
        etot = etot + 2.0d0*EtsvdW
        hwf_energy = hwf_energy + 2.0d0*EtsvdW
     END IF
     ! add the many-body dispersion energy contribution to the total
     IF (mbd_vdw) THEN
        WRITE(stdout,'(7X,"Adding MBD energy: ", F9.6," to total")') EmbdvdW
  ...
  CONTAINS
     ...
     SUBROUTINE print_energies ( )
       ...
       IF ( ( conv_elec .OR. MOD( iter, iprint ) == 0 ) .AND. .NOT. lmd ) THEN
          ...
          IF ( ts_vdw )  WRITE ( stdout , 9076 ) 2.0d0*EtsvdW ! convert from Hartree to Ry
          IF ( mbd_vdw ) WRITE ( stdout , 9076 ) 2.0d0*EmbdvdW  ! return in Hartree
          !
