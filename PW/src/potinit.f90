...
SUBROUTINE potinit()
  ...
  USE control_flags,        ONLY : lscf, ts_vdw
  ...
  USE tsvdw_module,         ONLY : vdw_lstep0 !HK
  !
  IMPLICIT NONE
  !
  ...
  IF ( dft_is_meta() ) then
     ...
     DO is = 1, nspin
        ...
        IF ( .not. exst ) rho%kin_r(:,is) = fact * abs(rho%of_r(:,is)*nspin)**(5.0/3.0)/nspin
        psic(:) = rho%kin_r(:,is)
        CALL fwfft ('Dense', psic, dfftp)
