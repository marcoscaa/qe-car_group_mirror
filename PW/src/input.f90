...
SUBROUTINE iosys()
  ...
  USE input_parameters, ONLY : ibrav, celldm, a, b, c, cosab, cosac, cosbc, &
                               ...
                               mbd_vdw, mbd_vdw_isolated, mbd_vdw_econv_thr,  &
                               mbd_vdw_vacuum, mbd_vdw_supercell,             &
                               mbd_vdw_n_quad_pts, mbd_vdw_forces,            &
                               mbd_vdw_beta, mbd_vdw_custom_beta,             &
                               vdw_self_consistent, dveff_off, vdw_debug,     &
                               vdw_small_box,                                 &
                               xdm, xdm_a1, xdm_a2,                           &
  ...
  SELECT CASE( TRIM( vdw_corr ) )
    ...
    CASE( 'TS', 'ts', 'ts-vdw', 'ts-vdW', 'tkatchenko-scheffler' )
      ...
      lxdm   = .FALSE.
      !
    CASE( 'MBD', 'mbd', 'mbd-vdw', 'mbd-vdW', 'many body dispersion', 'manybody dispersion' )
      !
      llondon= .FALSE.
      ts_vdw_= .FALSE.
      mbd_vdw_= .TRUE.
  ...
  IF ( mbd_vdw ) THEN
    ...
    mbd_vdw_ = .TRUE.
  END IF
  IF ( llondon.AND.lxdm .OR. llondon.AND.ts_vdw_ .OR. lxdm.AND.ts_vdw_ .OR. lxdm.AND.mbd_vdw_ ) &
  ...
  END IF
  IF ( mbd_vdw_ ) THEN
    vdw_isolated = mbd_vdw_isolated
    vdw_econv_thr= mbd_vdw_econv_thr
    vdw_small_box_ = vdw_small_box
  END IF
  !
