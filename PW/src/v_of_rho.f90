...
SUBROUTINE v_of_rho( rho, rho_core, rhog_core, &
  ...
  USE control_flags,    ONLY : ts_vdw, mbd_vdw, conv_elec
  USE tsvdw_module,     ONLY : tsvdw_calculate, UtsvdW
  USE tsvdw_module,     ONLY : vdw_lscreen, vdw_lstep0 !HK
!TM/HK: new
  USE io_global,        ONLY : stdout                          !print/write argument for standard output (to output file)
  USE mbdvdw_module,    ONLY : mbdvdw_calculate, dir
  USE mbdvdw_module,    ONLY : EmbdvdW, Umbdvdw, FmbdvdW, HmbdvdW
  use tsvdw_module,     ONLY : EtsvdW, HtsvdW, FtsvdW
  use mbdvdw_module,    ONLY : do_forces
  ...
  use input_parameters,   ONLY: mbd_vdw_forces, vdw_debug
!TM/HK: new
  !
  IMPLICIT NONE
  !
  ...
  ! ! 
  INTEGER :: is, ir, s, i
  real(dp) :: tau_temp(3, nat), num_grad(3, nat), plus, minus, h, at_temp(3,3), ainv_temp(3,3), num_dh(3,3)
    ...
    !
     if(vdw_debug) then
       ...
       do s = 1, 3, 1
         do i = 1, 3, 1
           at_temp = at*alat
           at_temp(s, i) = at_temp(s, i) + h
           call inv3x3_mat(at_temp, ainv_temp)
           IF (conv_elec) vdw_lstep0 = .TRUE.
           CALL tsvdw_calculate(tau*alat,rho%of_r,at_temp,ainv_temp)
           IF (.not.conv_elec) vdw_lscreen=.FALSE.
           plus = EtsvdW
           write(stdout, *) s, i, '+', plus
           ...
           CALL tsvdw_calculate(tau*alat,rho%of_r,at_temp,ainv_temp)
           minus = EtsvdW
           write(stdout, *) s, i, '+', minus

           num_dh(s, i) = (plus - minus)/(2.0_DP*h)
     ...
     !
     IF (conv_elec) vdw_lstep0 = .TRUE.
     CALL tsvdw_calculate(tau*alat,rho%of_r,at*alat,transpose(bg)/alat)
     IF (.not.conv_elec) vdw_lscreen=.FALSE.

     if(vdw_debug) then
       do s = 1, 3, 1
        do i = 1, 3, 1
          write(stdout, *) i, s, num_dh(i, s), HtsvdW(i, s)
          end do
        end do
        end if
  ...
  IF (mbd_vdw.and.conv_elec) THEN
    if(vdw_debug) then
      ...
      do i = 1, nat, 1
        do s = 1, 3, 1
          ...
          dir = "benzene_"//displ//"_"//trim(x1)//"_plus/"
          ! print*, dir
          tau_temp = tau*alat
          tau_temp(s, i) = tau_temp(s, i) + h
          CALL mbdvdw_calculate(tau_temp,rho%of_r,at*alat,transpose(bg)/alat)
          plus = EmbdvdW
          write(stdout, *) i, s, '+', plus

          dir = "benzene_"//displ//"_"//trim(x1)//"_minus/"
          ! print*, dir
          tau_temp = tau*alat
          tau_temp(s, i) = tau_temp(s, i) - h
          CALL mbdvdw_calculate(tau_temp,rho%of_r,at*alat,transpose(bg)/alat)
          minus = EmbdvdW
          write(stdout, *) i, s, '-', minus

          num_grad(s, i) = -1.0d0*(plus-minus)/(2.0d0*h)

          ! do is = 1, nat, 1
          !   print*, tau_temp(1, is), tau_temp(2, is), tau_temp(3, is)
          !   end do
    ...
    CALL mbdvdw_calculate(tau*alat,rho%of_r,at*alat,transpose(bg)/alat)

    if(vdw_debug) then
      do i = 1, nat, 1
        do s = 1, 3, 1
          write(stdout, *) i, s, FmbdvdW(i, s), num_grad(s, i)
        end do
      end do
    end if
    ...
    ! DO is = 1, nspin_lsda
    !   DO ir=1,dfftp%nnr
    !     v%of_r(ir,is)=v%of_r(ir,is)+2.0d0*UmbdvdW(ir)
    !     END DO
    ! END DO
  END IF
  !
...
SUBROUTINE v_xc_meta( rho, rho_core, rhog_core, etxc, vtxc, v, kedtaur )
  ...
  &           v1xup, v1xdw, v1cup, v1cdw,                              &
  &           v3xup, v3xdw,v3cup, v3cdw, v2xup, v2xdw,                 &
  ...
  do k = 1, dfftp%nnr
     ...
     if (nspin == 1) then
        ...
        if ( (arho > eps8) .and. (grho2 (1) > eps12) .and. &
           ...
           !MCA/HK: TODO: check for a possible wrong factor of 2
           v(k, 1) =  (v1x + v1c )*e2 
           
           ! h contains D(rho*Exc)/D(|grad rho|) * (grad rho) / |grad rho|
           h(:,k,1) =  (v2x + v2c)*grho (:,k,1) *e2
           
           kedtaur(k,1)=  (v3x + v3c) * 0.5d0 * e2 !MCA/HK
           !kedtaur(k,1)=  (v3x + v3c) * e2 !MCA/HK
        ...
        call tau_xc_spin (rhoup, rhodw, grhoup, grhodw, tauup, taudw, ex, ec, &
          ...
          ! h contains D(rho*Exc)/D(|grad rho|) * (grad rho) / |grad rho|
          !
          if ( (get_meta()==1) .or. (get_meta()==4) ) then  ! tpss or scan functional
            !
          ...
          !kedtaur(k,1)=  (v3xup + v3cup) * e2
          !kedtaur(k,2)=  (v3xdw + v3cdw) * e2
          !
          etxc = etxc + (ex + ec) * e2
