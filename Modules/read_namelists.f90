...
!
       ...
       mbdvdw_and_tsvdw = .false.
       mbd_vdw          = .FALSE.
       mbd_vdw_isolated = .FALSE.
       vdw_self_consistent = .true. !HK/TM : for tsvdw and mbd self consistency
       mbd_vdw_econv_thr = 1.E-6_DP
       mbd_vdw_vacuum    = .false.
       mbd_vdw_supercell = 30.0_DP ! 30 Angstroms converted to Bohr.
       mbd_vdw_beta = 0.83
       mbd_vdw_custom_beta = .false.
       mbd_vdw_n_quad_pts = 20
       mbd_vdw_forces    = .true.
       vdw_debug=.false.
       dveff_off = .false.
       vdw_small_box = .FALSE. !HK
       xdm = .FALSE.
       xdm_a1 = 0.6836_DP
       xdm_a2 = 1.5045_DP
       !
...
!=======================================================================
     ...
     SUBROUTINE system_bcast()
       ...
       CALL mp_bcast( mbd_vdw_isolated,          ionode_id, intra_image_comm )
       CALL mp_bcast( vdw_self_consistent,   ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_custom_beta,       ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_beta,              ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_econv_thr,         ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_vacuum,            ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_supercell,         ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_n_quad_pts,        ionode_id, intra_image_comm )
       CALL mp_bcast( mbd_vdw_forces,            ionode_id, intra_image_comm )
       CALL mp_bcast( dveff_off,                 ionode_id, intra_image_comm )
       CALL mp_bcast( vdw_small_box,             ionode_id, intra_image_comm ) !HK
     ...
     SUBROUTINE electrons_bcast()
       ...
       CALL mp_bcast( dforce_emin,           ionode_id, intra_image_comm )
       !
       RETURN
       !
     END SUBROUTINE
...
!=========================================================================
          ...
          CASE ( 'vc-md', 'vc-cp' )
             ...
             END IF
             !
          CASE ('driver', 'driver-vc')
             !some parameters can be set here
             !
          CASE ('driver-cp-wf', 'driver-vc-cp-wf')
             !some parameters can be set here
             IF( prog == 'PW' ) &
                CALL errore( sub_name, ' calculation ' // &
                           & TRIM( calculation ) // ' not implemented ', 1 )
     ...
     SUBROUTINE read_namelists( prog, unit )
       ...
       IF ( ionode ) THEN
          ...
          IF ( TRIM( calculation ) == 'relax'    .OR. &
               ...
               TRIM( calculation ) == 'driver-vc'.OR. &
               TRIM( calculation ) == 'driver-cp-wf'    .OR. &
               TRIM( calculation ) == 'driver-vc-cp-wf' .OR. &
       ...
       IF( ionode ) THEN
          IF( TRIM( calculation ) == 'cp-wf'       .OR. &
              ...
              TRIM( calculation ) == 'cp-wf-nscf'  .OR. & 
              TRIM( calculation ) == 'driver-cp-wf'.OR. &
              TRIM( calculation ) == 'driver-vc-cp-wf') THEN
