...
!
  ...
  !              "scan"  = "???+??+scan+scan"  = SCAN Meta-GGA ! HK/MCA todo check
  !              "scan0" = "???+??+scan0+scan0"  = SCAN0       ! HK/MCA todo check
  !              "scan00 = "???+??+scan00+scan00"= SCAN00      ! HK/MCA todo check
  ...
  !              "scan"   SCAN Meta-GGA                  imeta=4 ! HK/MCA
  !              "scan0   SCAN0  hybrid Meta-GGA         imeta=5 ! HK/MCA
  !              "scan00  SCAN00 hybrid Meta-GGA         imeta=6 ! HK/MCA
  !
  ...
  !              scan    J Sun, A Ruzsinszky, and J Perdew, PRL 115, 36402 (2015) ! HK/MCA
  !              scan0   K Hui and J-D. Chai, JCP 144, 44114 (2016).              ! HK/MCA
  ...
  !
  integer, parameter:: nxc=8, ncc=10, ngcx=27, ngcc=12, nmeta=6, ncnl=3
  character (len=4) :: exc, corr, gradx, gradc, nonlocc
  character (len=6) :: meta !HK
  ...
  data meta  / 'NONE', 'TPSS', 'M06L', 'TB09', 'SCAN', 'SCAN0',&
    &          'SCAN00' /  !HK/MCA
...
CONTAINS
  ...
  subroutine set_dft_from_name( dft_ )
    ...
    ! special case : SCAN meta-GGA Exc !HK/MCA
    else IF ('SCAN'.EQ. TRIM(dftout ) ) THEN
       dft_defined = set_dft_values(0,0,0,0,0,4)

    ! special case : SCAN0 !HK/MCA
    else IF ('SCAN0'.EQ. TRIM(dftout ) ) THEN
       dft_defined = set_dft_values(0,0,0,0,0,5)

    ! special case : SCAN00 !HK/MCA
  ...
  subroutine set_auxiliary_flags
    ...
    ! HK/MCA: SCAN0
    IF ( imeta ==5 ) exx_fraction = 0.25_DP
    ! HK/MCA: SCAN00
    IF ( imeta ==6 ) exx_fraction = 0.50_DP
    ! HSE
    IF ( igcx ==12 ) THEN
 ...
 !-----------------------------------------------------------------------
  ...
  if (imeta_ == 1 ) then
     ...
     shortname_ = 'TB09'
  else if (imeta_ == 4) then !HK/MCA
     shortname_ = 'SCAN'
  else if (imeta_ == 5) then !HK/MCA
     shortname_ = 'SCAN0'
  else if (imeta_ == 6) then !HK/MCA
     shortname_ = 'SCAN00'
  end if
...
subroutine tau_xc (rho, grho, tau, ex, ec, v1x, v2x, v3x, v1c, v2c, v3c)
  ...
  elseif (imeta == 4) then ! HK/MCA: SCAN
     call  scancxc (rho, grho, tau, ex, ec, v1x, v2x, v3x, v1c, v2c, v3c)
  elseif (imeta == 5.or.imeta == 6) then ! HK/MCA: SCAN0 or SCAN00
     call  scancxc (rho, grho, tau, ex, ec, v1x, v2x, v3x, v1c, v2c, v3c)
     if (exx_started) then
        ex  = (1.0_DP - exx_fraction) * ex
        v1x = (1.0_DP - exx_fraction) * v1x
...
subroutine tau_xc_array (nnr, rho, grho, tau, ex, ec, v1x, v2x, v3x, v1c, v2c, v3c)
  ...
  elseif (imeta == 5.or.imeta == 6) then ! HK/MCA: SCAN0 or SCAN00
     ...
     call errore('v_xc_meta_array','array mode only works for SCAN',1)
  end if
...
!-----------------------------------------------------------------------
  ...
  real(dp), intent(out)               :: ex, ec, v1xup, v1xdw, v3xup, v3xdw,  &
                                      &  v1cup, v1cdw, v3cup, v3cdw, v2xup, v2xdw 
  ...
  !MCA/HK: for SCAN, this will be calculated later
  if (imeta /= 4) then
  ...
  elseif (imeta == 4) then !MCA/HK: SCAN, not the most efficient use of libxc 

     !call errore('v_xc_meta','SCAN still not implemented with spin polarized',1)
