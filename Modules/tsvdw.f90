...
USE control_flags,      ONLY: mbd_vdw
USE control_flags,      ONLY: mbdvdw_and_tsvdw
use input_parameters,   ONLY: dveff_off
...
USE funct,              ONLY: get_meta           !retrieves type of Meta-GGA functionals ! HK/MCA
...
LOGICAL, PUBLIC  :: vdw_lscreen=.TRUE.                       !control on determining atomic overlap
LOGICAL, PUBLIC  :: vdw_lstep0 =.TRUE.                       !logical for if the step is the initial step
LOGICAL, PUBLIC  :: vdw_small_box =.FALSE.                   !logical for turning on wrapping around !HK
...
INTEGER, PRIVATE  :: n_PBC_img_grid_atom                     !HK : number of images needed to wrap around
INTEGER, PRIVATE  :: n_PBC_img_atom_atom                     !HK : number of images needed to wrap around
...
INTEGER, DIMENSION(:), ALLOCATABLE, public :: NsomegaA      !number of points in the spherical atomic integration domain
...
INTEGER, DIMENSION(:,:), ALLOCATABLE, PRIVATE :: wrp_img_grid_atom     !ordered wrap around images (1:3, n_PBC_img_grid_atom) !HK
INTEGER, DIMENSION(:,:), ALLOCATABLE, PRIVATE :: wrp_img_atom_atom     !ordered wrap around images (1:3, n_PBC_img_atom_atom) !HK
INTEGER, DIMENSION(:,:), ALLOCATABLE, PRIVATE :: BsomegaAr !boundary number of points in the reduced spherical atomic integration domain with respect to the wrap arround images !HK
INTEGER, DIMENSION(:,:,:), ALLOCATABLE, public :: somegaA   !spherical atomic integration domain
...
REAL(DP), DIMENSION(:), ALLOCATABLE, public :: vfree        !free atomic volumes for each atomic species
...
REAL(DP), DIMENSION(:,:), ALLOCATABLE, public:: dveffAdn    !the local copy of the TS-vdW wavefunction forces (dispersion potential)
...
REAL(DP), DIMENSION(:,:,:), ALLOCATABLE, public :: dveffdR  !first derivative of effective volume wrt nuclear displacement
REAL(DP), DIMENSION(:,:,:), ALLOCATABLE, public :: dveffdh  !first derivative of effective volume wrt cell displacement
real(dp), dimension(3,3) :: Htmp
!
! PUBLIC subroutines
!
!  11/18 Thomas promoted GetVdWParam, tsvdw_veff, tsvdw_dveff
!             so that we could use them in the MBDVDW module
PUBLIC :: tsvdw_initialize
PUBLIC :: tsvdw_calculate
PUBLIC :: tsvdw_cleanup
PUBLIC :: tsvdw_finalize
PUBLIC :: GetVdWParam
PUBLIC :: tsvdw_veff
PUBLIC :: tsvdw_dveff
PUBLIC :: tsvdw_wfforce
public :: tsvdw_cleanup_post_mbd
!
! PRIVATE subroutines
!
...
PRIVATE :: Num1stDer
PRIVATE :: CubSplCoeff
PRIVATE :: wrap_around_image_check
  ...
  !
  Ndim=MAX(nr1*nr2,dfftp%nnr)
  ALLOCATE(UtsvdW(Ndim)); UtsvdW=0.0_DP
  !ALLOCATE(UtsvdW(dfftp%nnr)); UtsvdW=0.0_DP !HK: todo might be modified to dffts compare (see also ../PW/src/v_of_rho.f90)
  !
  ...
  ELSE IF (get_iexch().EQ.6.AND.get_icorr().EQ.4.AND.get_igcx().EQ.8.AND.get_igcc().EQ.4) THEN
    ...
    !
  ELSE IF (get_meta().EQ.4) THEN
    !
    sR=1.17_DP !SCAN !HK/MCA : fitted with S22 testset using the right wf force
    !
  ELSE IF (get_meta().EQ.5) THEN
    !
    sR=1.17_DP !SCAN0 !HK/MCA : to be tested more... (this is scan value---just to make the testing of SCAN0 easy) FIXME
    !
  ELSE IF (get_meta().EQ.6) THEN
    !
    sR=1.17_DP !SCAN00 !HK/MCA : to be tested more... (this is scan value---just to make the testing of SCAN00 easy) FIXME
    !
  ELSE
    !
  ...
  !
  !--------------------------------------------------------------------------------------------------------------
  SUBROUTINE tsvdw_calculate(tauin, rhor, at_x_alat, bg_trans_by_alat)
  !--------------------------------------------------------------------------------------------------------------
  ...
  IMPLICIT NONE
!debug
    integer, parameter :: F_outfile = 20
    integer :: i_atom, s, i
!debug
  ...
  ! HK: fail-safe check
  !
  if (.not.allocated(gomegaAr)) vdw_lscreen =.true.
  !
  ! HK: wrapper for the cell
  !
...
!debug
!IF(vdw_lscreen) then
!  write(stdout,*) 'screen on'
!else
!  write(stdout,*) 'screen off'
!end if
!debug
  !
  ! Compute unique atom pair list...
  !
  IF(vdw_lscreen) CALL tsvdw_unique_pair()
  !
  ...
  !
  IF(vdw_lscreen) CALL tsvdw_screen()
  !
...
!TM/HK: debug
!  write(stdout, *), dveff_off
!TM/HK: debug

  if(dveff_off) write(stdout, *) "Turning dveff off by setting dveff=0.0_DP"

  if(dveff_off) dveffdr = 0.0_DP
  if(dveff_off) dveffdh = 0.0_DP
  ...
  if(.not.mbd_vdw) then
    ...
    !
    if(vdw_self_consistent) CALL tsvdw_wfforce() !HK/RAD/TM: turn on/off self consistency
  end if
!debug
!    Htmp = 1.0_DP*MATMUL( HtsvdW, transpose(h_) )/omega
!
!    write(stdout, '(3X, "HtsVdW")')
!    do s = 1, 3, 1
...
!debug
  ...
  !     open(unit=F_outfile,file="TS_ion_forces.tmp",action="write",status="unknown",position="append")
  !       do s=1,nat,1
  !           do i=1,3,1
  !               write(F_outfile, *) s, i, FtsvdW(i, s)
  !           end do
  !       end do
  !     close(F_outfile)
  !     end if
...
10  DO j=1,nproc_image
  ...
  DO ia = 1, nat
    !
    atxyzs(1,ia)=ainv_(1,1)*tauin(1,ia)+ainv_(1,2)*tauin(2,ia)+ainv_(1,3)*tauin(3,ia)   ! s = h^-1 r
    atxyzs(2,ia)=ainv_(2,1)*tauin(1,ia)+ainv_(2,2)*tauin(2,ia)+ainv_(2,3)*tauin(3,ia)   ! s = h^-1 r
    atxyzs(3,ia)=ainv_(3,1)*tauin(1,ia)+ainv_(3,2)*tauin(2,ia)+ainv_(3,3)*tauin(3,ia)   ! s = h^-1 r
    !
    ...
    !
    atxyz(1,ia)=h_(1,1)*atxyzs(1,ia)+h_(1,2)*atxyzs(2,ia)+h_(1,3)*atxyzs(3,ia)   ! r = h s
    atxyz(2,ia)=h_(2,1)*atxyzs(1,ia)+h_(2,2)*atxyzs(2,ia)+h_(2,3)*atxyzs(3,ia)   ! r = h s
    atxyz(3,ia)=h_(3,1)*atxyzs(1,ia)+h_(3,2)*atxyzs(2,ia)+h_(3,3)*atxyzs(3,ia)   ! r = h s
    !
  END DO
  !
  ...
  ! Local variables
  !
  INTEGER :: ia,ib,ias,ibs,ip,ir,i,j,k,jj,nj_max,num,num1,jj_neib_of_i
  ...
  !
  if(.not.allocated(wrp_img_atom_atom)) call wrap_around_image_check(2*maxval(spdata(:,1)),1) !HK: go to bigest cutoff for simplicity (checked energy with silicon case)
  ...
  DO ia=1,nat
    ...
    DO ib=1,nat
      ...
      IF(ib.NE.ia) THEN
        ...
        !
        dAB2(1)=ainv_(1,1)*dAB(1)+ainv_(1,2)*dAB(2)+ainv_(1,3)*dAB(3)   ! s_AB = h^-1 r_AB
        dAB2(2)=ainv_(2,1)*dAB(1)+ainv_(2,2)*dAB(2)+ainv_(2,3)*dAB(3)   ! s_AB = h^-1 r_AB
        dAB2(3)=ainv_(3,1)*dAB(1)+ainv_(3,2)*dAB(2)+ainv_(3,3)*dAB(3)   ! s_AB = h^-1 r_AB
        !
        ...
        !
        dAB(1)=h_(1,1)*dAB2(1)+h_(1,2)*dAB2(2)+h_(1,3)*dAB2(3)   ! r_AB = h s_AB (MIC)
        dAB(2)=h_(2,1)*dAB2(1)+h_(2,2)*dAB2(2)+h_(2,3)*dAB2(3)   ! r_AB = h s_AB (MIC)
        dAB(3)=h_(3,1)*dAB2(1)+h_(3,2)*dAB2(2)+h_(3,3)*dAB2(3)   ! r_AB = h s_AB (MIC)
        !
...
40    CONTINUE
  ...
  INTEGER :: i_PBC_img           !HK : index of wrap around images
  INTEGER :: img_a, img_b, img_c !HK: boundary and index of images (for small cell case) (tobe replaced by the above)
  ...
  !
  ! HK: allocate the boundary array for later wrap arround usage
  !
  if(.not.allocated(wrp_img_grid_atom)) call wrap_around_image_check(maxval(spdata(:,1)),2) !HK: go to bigest cutoff for simplicity (checked energy with silicon case)
  !
  if(.not.allocated(BsomegaAr)) then
    ALLOCATE(BsomegaAr(n_PBC_img_grid_atom,nat)); BsomegaAr=0
  end if
  !
...
!debug
!write(stdout,*) 'HK: Ntmp, omega, nr1, nr2, nr3', Ntmp, omega, nr1, nr2, nr3
!write(stdout,*) 'h'
!write(stdout,"(9F7.3)") h(:,:)
!write(stdout,*) 'h*ainv'
!write(stdout,"(9F7.3)") matmul(h,ainv)
!write(stdout,*) 'h_'
!write(stdout,"(9F7.3)") h_(:,:)
!write(stdout,*) 'h_*ainv_'
!write(stdout,"(9F7.3)") matmul(h_,ainv_)
!debug
  !
  ...
  DO iproc=1,nstates(me)
    ...
    !
    !
    ! HK: add images if necessary (to get all points within atomic radial grid cutoff accounted for)
    !
    do i_PBC_img = 1, n_PBC_img_grid_atom
    img_a = wrp_img_grid_atom(1,i_PBC_img)
    img_b = wrp_img_grid_atom(2,i_PBC_img)
    img_c = wrp_img_grid_atom(3,i_PBC_img)
    !
    ...
    DO ir1=1,nr1
      ...
      DO ir2=1,nr2
        ...
        DO ir3=1,nr3
          ...
          !
          dqA(1)=h_(1,1)*dq(1)+h_(1,2)*dq(2)+h_(1,3)*dq(3)   ! r_i = h s_i
          dqA(2)=h_(2,1)*dq(1)+h_(2,2)*dq(2)+h_(2,3)*dq(3)   ! r_i = h s_i
          dqA(3)=h_(3,1)*dq(1)+h_(3,2)*dq(2)+h_(3,3)*dq(3)   ! r_i = h s_i
          !
          dqA(1)=dqA(1)-atxyz(1,ia)   ! r_iA = r_i - r_A
          dqA(2)=dqA(2)-atxyz(2,ia)   ! r_iA = r_i - r_A
          dqA(3)=dqA(3)-atxyz(3,ia)   ! r_iA = r_i - r_A
          !
          dqAs(1)=ainv_(1,1)*dqA(1)+ainv_(1,2)*dqA(2)+ainv_(1,3)*dqA(3)   ! s_iA = h^-1 r_iA
          dqAs(2)=ainv_(2,1)*dqA(1)+ainv_(2,2)*dqA(2)+ainv_(2,3)*dqA(3)   ! s_iA = h^-1 r_iA
          dqAs(3)=ainv_(3,1)*dqA(1)+ainv_(3,2)*dqA(2)+ainv_(3,3)*dqA(3)   ! s_iA = h^-1 r_iA
          !
          ...
          !
          dqA(1)=h_(1,1)*dqAs(1)+h_(1,2)*dqAs(2)+h_(1,3)*dqAs(3)   ! r_iA = h s_iA (MIC)
          dqA(2)=h_(2,1)*dqAs(1)+h_(2,2)*dqAs(2)+h_(2,3)*dqAs(3)   ! r_iA = h s_iA (MIC)
          dqA(3)=h_(3,1)*dqAs(1)+h_(3,2)*dqAs(2)+h_(3,3)*dqAs(3)   ! r_iA = h s_iA (MIC)
          !
          dqA(:)=dqA(:)+h_(:,1)*dble(img_a)+h_(:,2)*dble(img_b)+h_(:,3)*dble(img_c)  ! HK: r_iA = h s_iA include images possibly inside spcutA
          !
          ! HK: dqAmic is now the distance from a grid q to the MIC atom A (img_abc==0) or the image A' (img_abc \= 0)
          !
          dqAmic(ir1,ir2,ir3)=DSQRT(dqA(1)*dqA(1)+dqA(2)*dqA(2)+dqA(3)*dqA(3))   ! |r_i - r_A| (MIC) !HK: could be images if img_abc not 0
          !
          ...
          IF (dqAmic(ir1,ir2,ir3).LE.spcutA) THEN
            ...
            !
            ! Form numerator of dVA/dn(r) only... !HK: to a particular MIC atom A and its nearby images
            !
...
!$omp end parallel do
    ...
    END DO !ir1
    !
    !HK: store the boundary in order (according the the nested loops)
    BsomegaAr(i_PBC_img,ia) = Nptsr
    !
    !END DO ! img_c
    !END DO ! img_b
    !END DO ! img_a
  ...
  CALL mp_sum(BsomegaAr,intra_image_comm) !HK
  !
...
!$omp parallel do private(off1)
    DO iq=1,NsomegaA(ia)
      !
      ! Compute complete dispersion potential (functional derivative of veff(A) wrt charge density)...
      !
...
!$omp parallel do private(off1),reduction(+:veff)
    DO iq=1,NsomegaA(ia)
      !
      ! Compute veff integrand and increment veff ...
      !
...
!$omp end parallel do
  ...
  REAL(DP) :: spcutA,spcutB,spdA,spdB,dq(3),dqA(3),dqAs(3),dqB(3),dqBs(3),dqAmic,dqBmic,normr
  ...
  REAL(DP) :: weight_dveff           !HK : wrapper for the weight of intra / cross PBC image
  INTEGER :: img_a, img_b, img_c     !HK
  INTEGER :: ia_PBC_img, ib_PBC_img  !HK : index of wrap around images
  INTEGER :: i_PBC_img_atom          !HK : image for atom wrapping
  INTEGER :: num_pair                !HK : number of pair in this image
  INTEGER :: pair_list(nat)          !HK : pair list wraper
  INTEGER :: tt                      !HK : counter
  REAL(DP):: dAB(3)                  !HK : atom A B distance (including image wrapping back)
  REAL(DP):: dAB2(3)                 !HK : atom A B distance (including image wrapping back)
  REAL(DP):: dABimg                  !HK : atom A B distance (including image wrapping back)
  REAL(DP):: dABimg2                 !HK : square of dABimg
  !
...
!$omp off1,dptmp1,dptmp2,dptmp3,dptmp4,dVAdRA,dptmp5,i,j,ia_PBC_img), &
!$omp reduction(-:dveffdh),reduction(+:dveffdR)
    DO iq=1,NsomegaAr(ia)
      ...
      dqAs(3)=dqAs(3)-IDNINT(dqAs(3))   ! impose MIC on s_iA in range: [-0.5,+0.5]
      !
      ! HK: current PBC image
      !! !!N.B. todo omp serial
      !do ia_PBC_img = 1, n_PBC_img_grid_atom
      !  if (BsomegaAr(ia_PBC_img,ia).ge.iq) exit ! HK : found current ia_PBC_img
      !end do ! ia_PBC_img
ia_PBC_img = PBC_img_of_grid(iq,ia)
      !
      ! HK: do the wrap around (gird)
      !
      dqAs(1)=dqAs(1)+dble(wrp_img_grid_atom(1,ia_PBC_img))
      dqAs(2)=dqAs(2)+dble(wrp_img_grid_atom(2,ia_PBC_img))
      dqAs(3)=dqAs(3)+dble(wrp_img_grid_atom(3,ia_PBC_img))
      !
      ...
      dqA(3)=h_(3,1)*dqAs(1)+h_(3,2)*dqAs(2)+h_(3,3)*dqAs(3)   ! r_iA = h s_iA (MIC)
      !
      dqAmic=DSQRT(dqA(1)*dqA(1)+dqA(2)*dqA(2)+dqA(3)*dqA(3))   ! |r_i - r_A| (MIC) !HK: dqAmic is MIC + image translation
      !
      ! Determine the index in the atomic linear equispaced grid such that grd(ir) <= dqA <= grd(ir+1) and distance between dqA and grd(ir)...
      !
      ir=INT(dqAmic*spdA)
      ...
      DO i=1,3
        ...
        DO j=1,3
          !
          dveffdh(ia,i,j)=dveffdh(ia,i,j)-dVAdRA(i)*dqAxyzs(iq,j) !HK: this term is OK tested with Rn test case k444 vs super cell 211 (k244)
          !
        END DO !j
        !
      END DO !i
...
!$omp end parallel do
    !
    ! HK: wrap around B atom back. Since we were fixing B and wraping A, here we should use minus transform (fixing A)
    !
do i_PBC_img_atom = 1, n_PBC_img_atom_atom
img_a = wrp_img_atom_atom(1,i_PBC_img_atom)
img_b = wrp_img_atom_atom(2,i_PBC_img_atom)
img_c = wrp_img_atom_atom(3,i_PBC_img_atom)
    !
    !HK: center image (use unique pair) or other image (use all possible pair from the center)
    !
    if (img_a.eq.0.and.img_b.eq.0.and.img_c.eq.0) then
      num_pair = npair(ia)
      pair_list(:) = pair(:,ia)
      weight_dveff = 1.0_DP  !HK: both ia and ib counts
    else
      num_pair = nat
      do tt = 1,nat
        pair_list(tt) = tt
...
!$omp i,j,ib,ibs,spcutB,spdB,ioff,boff,ib_PBC_img), &
!$omp reduction(+:dveffdR),reduction(-:dveffdh)
    DO ipair=1, num_pair !HK
    !DO ipair=1,npair(ia)
      !
      ...
      spcutB=spdata(ibs,1)
      !
      ! HK: check if the pair is too far away or not
      !
      dAB(1)=atxyz(1,ia)-atxyz(1,ib)   ! r_AB = r_A - r_B
      dAB(2)=atxyz(2,ia)-atxyz(2,ib)   ! r_AB = r_A - r_B
      dAB(3)=atxyz(3,ia)-atxyz(3,ib)   ! r_AB = r_A - r_B
      ...
      dAB2(3)=dAB2(3)-IDNINT(dAB2(3))   ! impose MIC on s_AB in range: [-0.5,+0.5]
      ! wraping
      dAB2(1)=dAB2(1)+dble(img_a)
      dAB2(2)=dAB2(2)+dble(img_b)
      dAB2(3)=dAB2(3)+dble(img_c)
      !
      dAB(1)=h_(1,1)*dAB2(1)+h_(1,2)*dAB2(2)+h_(1,3)*dAB2(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
      dAB(2)=h_(2,1)*dAB2(1)+h_(2,2)*dAB2(2)+h_(2,3)*dAB2(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
      dAB(3)=h_(3,1)*dAB2(1)+h_(3,2)*dAB2(2)+h_(3,3)*dAB2(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
      !
      dABimg2=dAB(1)*dAB(1)+dAB(2)*dAB(2)+dAB(3)*dAB(3)
      dABimg=DSQRT(dABimg2)
      !
      if (dABimg.gt.(spcutA+spcutB)) cycle !HK : if too far there is no way to overlap
      !
      ! Precompute inverse of species-specific linear grid spacing (replaces / with * during interpolation)...
      !
      ...
      DO iq=1,NsomegaAr(ia)
        ...
        IF (BTEST(gomegaAr(iq,ioff,iproc),boff)) THEN
          ...
          !
          dqBs(1)=ainv_(1,1)*dqB(1)+ainv_(1,2)*dqB(2)+ainv_(1,3)*dqB(3)   ! s_iB = h^-1 r_iB
          dqBs(2)=ainv_(2,1)*dqB(1)+ainv_(2,2)*dqB(2)+ainv_(2,3)*dqB(3)   ! s_iB = h^-1 r_iB
          dqBs(3)=ainv_(3,1)*dqB(1)+ainv_(3,2)*dqB(2)+ainv_(3,3)*dqB(3)   ! s_iB = h^-1 r_iB
          !
          ...
          ! HK: current PBC image
          !
ib_PBC_img = PBC_img_of_grid(iq,ia)
          !! todo omp non safe -> use a function instead
          !do ib_PBC_img = 1, n_PBC_img_grid_atom
          !  if (BsomegaAr(ib_PBC_img,ia).ge.iq) exit ! HK : found current ib_PBC_img
          !end do ! ia_PBC_img
          !
          ! HK: do the wrap around back (grid)
          !
          dqBs(1)=dqBs(1)+dble(wrp_img_grid_atom(1,ib_PBC_img))
          dqBs(2)=dqBs(2)+dble(wrp_img_grid_atom(2,ib_PBC_img))
          dqBs(3)=dqBs(3)+dble(wrp_img_grid_atom(3,ib_PBC_img))
          !
          ! HK: do the wrap around back (atom)
          !
          dqBs(1)=dqBs(1)+dble(img_a)
          dqBs(2)=dqBs(2)+dble(img_b)
          dqBs(3)=dqBs(3)+dble(img_c)
          !
          dqB(1)=h_(1,1)*dqBs(1)+h_(1,2)*dqBs(2)+h_(1,3)*dqBs(3)   ! r_iB = h s_iB (MIC)
          dqB(2)=h_(2,1)*dqBs(1)+h_(2,2)*dqBs(2)+h_(2,3)*dqBs(3)   ! r_iB = h s_iB (MIC)
          dqB(3)=h_(3,1)*dqBs(1)+h_(3,2)*dqBs(2)+h_(3,3)*dqBs(3)   ! r_iB = h s_iB (MIC)
          !
          dqBmic=DSQRT(dqB(1)*dqB(1)+dqB(2)*dqB(2)+dqB(3)*dqB(3))   ! |r_i - r_B| (MIC)  !HK: dqBmic is MIC + image translation
          !
          ...
          IF (dqBmic.LE.spcutB) THEN
            ...
            DO i=1,3
              ...
              dveffdR(ib,ia,i)=dveffdR(ib,ia,i)+dVBdRA(i)*weight_dveff !HK: prevent double counting in wrap around case (weight_dveff)
              !
            END DO !i
            !
            ...
            DO i=1,3
              ...
              DO j=1,3
                ...
                dveffdh(ib,i,j)=dveffdh(ib,i,j)-dVBdRA(i)*dqAxyzs(iq,j)*weight_dveff !HK: prevent double counting in wrap around case (weight_dveff)
                !
              END DO !j
              !
            END DO !i
...
!do ia = 1,nat
!do ib= 1,nat
!  write(1000,"(3F7.3)")dveffdh(ia,ib,:)
!end do ! ib
!end do ! ia
!write(2000,*)"dveffdh"
!do ia = 1,nat
!  write(2000,"(9F14.10)")dveffdh(ia,:,:)
!end do ! iproc
!call flush_unit(1000)
!call flush_unit(2000)
!!HK: debug
      ...
      DO ib=1,nat
          ...
          DO n2=-n_period,n_period
            ...
            DO n3=-n_period,n_period
              ...
              IF ((ABS(n1).EQ.n_period).OR.(ABS(n2).EQ.n_period).OR.(ABS(n3).EQ.n_period)) THEN
                ...
                !
                dAB(1)=h_(1,1)*dsAB(1)+h_(1,2)*dsAB(2)+h_(1,3)*dsAB(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
                dAB(2)=h_(2,1)*dsAB(1)+h_(2,2)*dsAB(2)+h_(2,3)*dsAB(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
                dAB(3)=h_(3,1)*dsAB(1)+h_(3,2)*dsAB(2)+h_(3,3)*dsAB(3)   ! r_AB = h s_AB (MIC only if n_period == 0)
                !
        ...
        IF (ia.NE.ib) THEN
          ...
          DO i=1,3
            ...
            !
            !HK: I think it is a bug (fixed with next do loop block) TODO: discussion
            !DO j=1,3
            !  !
        ...
        END IF
        !
        !HK: self image stress needs to be accounted for
        !
        DO i=1,3
          !
          DO j=1,3
...
!$omp end parallel
    ...
    !
    IF ( vdw_isolated .OR. (ABS(EtsvdW_period) <= vdw_econv_thr.and.n_period.gt.0) ) periodic_converged=.TRUE.
    !
    n_period=n_period+1
    !
...
!write(1000,*)"FtsvdW"
!do iproc = 1,nat
!  write(1000,*)FtsvdW(:,iproc)
!end do ! iproc
!write(2000,*)"HtsvdW"
!do iproc = 1,3
!  write(2000,*)HtsvdW(:,iproc)
!end do ! iproc
!call flush_unit(1000)
!call flush_unit(2000)
!write(3000,*)"FtsvdW"
!do iproc = 1,nat
!  write(3000,*)FtsvdW(:,iproc)
!end do ! iproc
!call flush_unit(3000)
!WRITE(4000,"(9F12.8)") (-1.d0 * (MATMUL( HtsvdW(:,:), TRANSPOSE(h_) )) / omega)
!call flush_unit(4000)
!!HK: debug
  !
  ! Deallocate temporary arrays...
  !
...
!$omp end parallel do
  ...
  if(.not.mbd_vdw) then
    ...
    IF(vdw_lstep0) THEN
      ...
      IF (ALLOCATED(wrp_img_grid_atom))     DEALLOCATE(wrp_img_grid_atom)   !HK
      IF (ALLOCATED(wrp_img_atom_atom))     DEALLOCATE(wrp_img_atom_atom)   !HK
      IF (ALLOCATED(BsomegaAr))   DEALLOCATE(BsomegaAr) !HK
  ...
  SUBROUTINE tsvdw_cleanup_post_mbd()
  !--------------------------------------------------------------------------------------------------------------
  !
  IMPLICIT NONE
  !
  ...
  !
  !--------------------------------------------------------------------------------------------------------------
  END SUBROUTINE tsvdw_cleanup_post_mbd
  !--------------------------------------------------------------------------------------------------------------
  !--------------------------------------------------------------------------------------------------------------
  ...
  !--------------------------------------------------------------------------------------------------------------
  !
subroutine  wrap_around_image_check(Rcut,property)
  !
  ! Author    : Hsin-Yu Ko
  ! Started at: 09.09.15
  !
  implicit none
  real(dp), intent(in) :: Rcut                    ! radial cutoff distance
  integer,  intent(in) :: property                ! 1: for atom-atom pair ; 2: for grid-atom pair
  !
  ! local var
  integer, parameter :: n_side = 50   ! generate a rough discrete sphere volume
  integer, allocatable :: img(:,:)
  integer, allocatable :: img_reduced(:,:)
  integer :: i, j, k
  integer :: i_img
  integer :: isp, jsp, n_sp ! n_sp : number of pts in sphere
  real(dp), allocatable :: sphere(:,:), ssphere(:,:)
  real(dp):: x, y, z, v_ceter(3)
  logical, allocatable :: excl(:)
  !
  ! init
  !
  if (property.eq.1) then
    n_PBC_img_atom_atom = 0
  else if (property.eq.2) then
    n_PBC_img_grid_atom = 0
  else
    call errore('tsvdw:wrap_around_image_check','unknown property to search for wrap around...',1)
  end if ! property
  !
  ! big box case
  !
  if (.not.vdw_small_box) then
    if (property.eq.1) then
      n_PBC_img_atom_atom = 1
      allocate(wrp_img_atom_atom(3,1)); wrp_img_atom_atom=0 !HK
    else if (property.eq.2) then
      n_PBC_img_grid_atom = 1
      allocate(wrp_img_grid_atom(3,1)); wrp_img_grid_atom=0 !HK
    end if
    !
    return
  end if ! .not.vdw_small_box
  !
  !
  allocate(sphere(3,(2*n_side+1)*(2*n_side+1)*(2*n_side+1))); sphere = 0._dp
  !
  ! generate uniform sphere
  !
  v_ceter(:) = 0.5_dp*(h_(:,1)+h_(:,2)+h_(:,3))
  !
  !HK: put the first point at the center so that the wrap around image will start from the center img
  !
  n_sp = 1
  sphere(:,n_sp) = v_ceter(:)
  !
  !HK: filter the remaining points with distance critrion
  !
  do i = -n_side,n_side
    x = dble(i)/dble(n_side)*Rcut
    do j = -n_side,n_side
      y = dble(j)/dble(n_side)*Rcut
      do k = -n_side,n_side
        z = dble(k)/dble(n_side)*Rcut
        if (sqrt(x*x+y*y+z*z).le.Rcut) then
          ! storage
          n_sp = n_sp + 1
          sphere(1,n_sp) = x
          sphere(2,n_sp) = y
          sphere(3,n_sp) = z
          !
          ! shift the center of the sphere to the center of the box
          !
          sphere(:,n_sp) = sphere(:,n_sp) + v_ceter(:)
          !if (n_sp.le.100) write(30,*) sphere(:,n_sp) !debug
        end if
      end do ! k
    end do ! j
  end do ! i
  !
  allocate(ssphere(3,n_sp));    ssphere = 0._dp
  allocate(img(3,n_sp));        img = 0
  allocate(img_reduced(3,n_sp));img_reduced = 0
  allocate(excl(n_sp));         excl = .false.
  !
  ! check which images are covered by the sphere points
  !
  do isp = 1, n_sp
    ssphere(1,isp)=ainv_(1,1)*sphere(1,isp)+ainv_(1,2)*sphere(2,isp)+ainv_(1,3)*sphere(3,isp)   ! s = h^-1 r
    ssphere(2,isp)=ainv_(2,1)*sphere(1,isp)+ainv_(2,2)*sphere(2,isp)+ainv_(2,3)*sphere(3,isp)   ! s = h^-1 r
    ssphere(3,isp)=ainv_(3,1)*sphere(1,isp)+ainv_(3,2)*sphere(2,isp)+ainv_(3,3)*sphere(3,isp)   ! s = h^-1 r
    !if (isp.le.100) write(30,*) ssphere(:,isp) !debug
    !
    do i = 1, 3
      !if (ssphere(i,isp).lt.0) then
      !  img(i,isp) = floor(ssphere(i,isp))
      !else
      !  img(i,isp) = int(ssphere(i,isp)) ! HK: floor
      !end if
      img(i,isp) = floor(ssphere(i,isp))
    end do ! i
    !if (isp.le.100) write(40,*) img(:,isp) !debug
  end do ! isp
  deallocate(sphere)
  !
  ! find non-repeating images
  !
  do isp = 1, n_sp
    if (.not.excl(isp)) then
      if (property.eq.1) then
        n_PBC_img_atom_atom = n_PBC_img_atom_atom + 1
        img_reduced(:,n_PBC_img_atom_atom) = img(:,isp)
        !
        ! mute similar images
        ! todo omp
        do jsp = isp+1, n_sp
          if ((.not.excl(jsp)).and.&
            & img(1,jsp).eq.img_reduced(1,n_PBC_img_atom_atom).and.&
            & img(2,jsp).eq.img_reduced(2,n_PBC_img_atom_atom).and.&
            & img(3,jsp).eq.img_reduced(3,n_PBC_img_atom_atom)) excl(jsp) = .true.
        end do ! jsp
      else if (property.eq.2) then
        n_PBC_img_grid_atom = n_PBC_img_grid_atom + 1
        img_reduced(:,n_PBC_img_grid_atom) = img(:,isp)
        !
        ! mute similar images
        ! todo omp
        do jsp = isp+1, n_sp
          if ((.not.excl(jsp)).and.&
            & img(1,jsp).eq.img_reduced(1,n_PBC_img_grid_atom).and.&
            & img(2,jsp).eq.img_reduced(2,n_PBC_img_grid_atom).and.&
            & img(3,jsp).eq.img_reduced(3,n_PBC_img_grid_atom)) excl(jsp) = .true.
      end do ! jsp
      end if !property
    end if
  end do ! isp
  !
  ! connect with the rest of the tsvdw
  !
  if (property.eq.1) then
    allocate(wrp_img_atom_atom(3,n_PBC_img_atom_atom)); wrp_img_atom_atom=0 !HK
    do i_img = 1, n_PBC_img_atom_atom
      wrp_img_atom_atom(:,i_img) = img_reduced(:,i_img)
      !write(20,*) wrp_img_atom_atom(:,i_img) !debug
    end do ! i_img
  else if (property.eq.2) then
    allocate(wrp_img_grid_atom(3,n_PBC_img_grid_atom)); wrp_img_grid_atom=0 !HK
    do i_img = 1, n_PBC_img_grid_atom
      wrp_img_grid_atom(:,i_img) = img_reduced(:,i_img)
      !write(20,*) wrp_img_grid_atom(:,i_img) !debug
    end do ! i_img
  end if ! property
  !
  ! deallocate
  ...
  deallocate(img)
  deallocate(excl)
  deallocate(img_reduced)
  deallocate(ssphere)
  !
  return
end subroutine wrap_around_image_check
!
function PBC_img_of_grid(iq,ia)
  !
  ! Author    : Hsin-Yu Ko
  ! Started at: 26.09.15
  !
  implicit none
  !
  integer, intent(in) :: iq, ia
  integer :: PBC_img_of_grid
  !
  integer :: img
  !
  do PBC_img_of_grid = 1, n_PBC_img_grid_atom
    if (BsomegaAr(PBC_img_of_grid,ia).ge.iq) exit ! HK : found current ib_PBC_img
  end do ! img
end function PBC_img_of_grid
  !
