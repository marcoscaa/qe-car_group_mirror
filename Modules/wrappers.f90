...
MODULE wrappers
  ...
  INTERFACE
    ...
    FUNCTION getcwd(buffer,size) BIND(C,name="getcwd") RESULT(r)
      ...
      TYPE(c_ptr)  :: r
    END FUNCTION
    SUBROUTINE load_balancing(neigh,nbsp,prev_load,overlap,load) BIND(C,name="load_balancing")
      USE iso_c_binding
      INTEGER(kind=c_int)                 :: neigh
      INTEGER(kind=c_int)                 :: nbsp
      INTEGER(kind=c_int)                 :: prev_load(*)
      INTEGER(kind=c_int)                 :: overlap(*)
      INTEGER(kind=c_int)                 :: load(*)
    END SUBROUTINE
  END INTERFACE
  !
  ! ====================================================================
