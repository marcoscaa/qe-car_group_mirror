...
!
        ...
        CHARACTER(len=80) :: calculation_allowed(19)
        DATA calculation_allowed / 'scf', 'nscf', 'relax', 'md', 'cp', &
          'vc-relax', 'vc-md', 'vc-cp', 'bands', 'neb', 'smd', 'cp-wf', &
          'vc-cp-wf', 'cp-wf-nscf', 'driver', 'driver-cp', 'driver-vc', &
          'driver-cp-wf', 'driver-vc-cp-wf' /
        ...
        NAMELIST / control / title, calculation, verbosity, restart_mode, &
          ...
          lcalc_z2, z2_m_threshold, z2_z_threshold, srvaddress
...
!
        ...
        logical :: mbd_vdw_custom_beta = .false.
        logical :: vdw_self_consistent = .true. !HK/TM : for tsvdw and mbd self consistency
        real(dp) :: mbd_vdw_beta = 0.83
        logical :: mbd_vdw_forces = .true.
        logical :: vdw_debug = .false.
        LOGICAL :: mbdvdw_and_tsvdw = .false.
        LOGICAL :: mbd_vdw_isolated = .FALSE.
        logical :: dveff_off = .false.
        logical, dimension(3) :: mbd_vdw_vacuum = .false.
        integer :: mbd_vdw_n_quad_pts = 0
        real(dp) :: mbd_vdw_supercell = 30.0_DP
          ! if .TRUE., MBD-vdW correction for isolated system
          ! if .FALSE., MBD-vdW correction for periodic system
        REAL(DP) :: mbd_vdw_econv_thr = 1.0E-6_DP
          ! convergence criterion for MBD-vdW energy for periodic system
          !
        LOGICAL :: vdw_small_box = .FALSE.
          ! if .TRUE., TS-vdW correction for small unit cell
          ! if .FALSE., TS-vdW correction for big unit cell
        ...
        NAMELIST / system / ibrav, celldm, a, b, c, cosab, cosac, cosbc, nat, &
             ...
             ts_vdw, ts_vdw_isolated, ts_vdw_econv_thr, mbdvdw_and_tsvdw,     &
             mbd_vdw, mbd_vdw_isolated, mbd_vdw_econv_thr, mbd_vdw_vacuum,    &
             mbd_vdw_forces, mbd_vdw_beta, mbd_vdw_custom_beta,               &
             vdw_self_consistent, dveff_off, vdw_small_box,                   &
             mbd_vdw_supercell, mbd_vdw_n_quad_pts, xdm, xdm_a1, xdm_a2,      &
...
!=----------------------------------------------------------------------------=!
        ...
        REAL(DP) :: conv_thr = 1.E-6_DP
          ...
          ! used only in PWscf
          ! used in electron_dynamics = 'CP-BO' in CP
       ...
       INTEGER  :: epol2 = 3
        ...
        REAL(DP) :: dforce_emin = 0.0_DP
          ! meaningful only if electron_dynamics = 'CP-BO'
          ! mean absolute deviation (MAD) in the changes in ionic forces between 
          ! successive electron minimization steps
        ...
        NAMELIST / electrons / emass, emass_cutoff, orthogonalization, &
          ...
          emass_emin, emass_cutoff_emin, electron_damping_emin,  &
          dt_emin, dforce_emin, efield_phase
