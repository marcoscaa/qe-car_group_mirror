...
SUBROUTINE punch_plot (filplot, plot_num, sample_bias, z, dz, &
  ...
  REAL(DP) :: charge_HK !HK : testing charge count
  REAL(DP) :: IPR       !HK : Inverse participation ratio
  INTEGER :: ibnd       !HK : band index
  ...
  ELSEIF (plot_num == 22) THEN
     !
     ! HK: compute the inverse partition ratio
     !
     if (ionode) open (unit=1321,file='IPR.dat')
     !
     do ibnd = 1, nbnd
       IF (noncolin) THEN
         IF (spin_component==0) THEN
           CALL local_dos (0, lsign, kpoint, ibnd, spin_component, emin, emax, raux)
         ELSE
           CALL local_dos_mag (spin_component, kpoint, ibnd, raux)
         ENDIF
       ELSE
         CALL local_dos (0, lsign, kpoint, ibnd, spin_component, emin, emax, raux)
       ENDIF
       ...
       ! HK: test integral
       if(ionode)then
         charge_HK = sum(raux1)*omega/(dfftp%nr1x*dfftp%nr2x*dfftp%nr3x)
         write(*,*) "HK: check integrated total charge of this band (should be 1)", charge_HK ! should be 1
         IPR = 1._DP/(sum(raux1*raux1)*omega*omega/(dfftp%nr1x*dfftp%nr2x*dfftp%nr3x))
         write(*,*) "HK: Inverse participation ratio for ibnd : (ibnd,IPR) = ", ibnd, IPR
         write(1321,*) IPR
       end if
#else
       ! HK: test integral
       if(ionode)then
         charge_HK = sum(raux)*omega/dfftp%nnr
         write(*,*) "HK: check integrated total charge of this band (should be 1)", charge_HK ! should be 1
         IPR = 1._DP/(sum(raux*raux)*omega*omega/dfftp%nnr)
         write(*,*) "HK: Inverse participation ratio for ibnd : (ibnd,IPR) = ", ibnd, IPR
         write(1321,*) IPR
       end if
#endif
